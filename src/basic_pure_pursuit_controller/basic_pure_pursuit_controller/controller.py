...
class PurePursuitController(Node):
    def __init__(self):
        ...
        self.publish_twist_stamped = bool(
            self.get_parameter("publish_twist_stamped").value
        )
        ...
        self.get_logger().info(
            f"Pure pursuit (single waypoint) running: Ld={self.Ld} m, v={self.v_des} m/s, w_max={self.w_max} rad/s"
        )
    ...
    def goal_cb(self, msg: PoseStamped):
        if (
            ...
            and msg.header.frame_id != self.odom_frame_id
        ):
            self.get_logger().warning(
                f'Goal frame "{msg.header.frame_id}" != odom frame "{self.odom_frame_id}".'
            )
            self.get_logger().info(
                f"Waiting for transform {msg.header.frame_id} to {self.odom_frame_id} ..."
            )
            transform = self.tf_buffer.lookup_transform(
                self.odom_frame_id, msg.header.frame_id, rclpy.time.Time()
            )
